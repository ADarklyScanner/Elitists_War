...
function finishBeginning(s: GameState) {
  ...
  s.phase = 'main';
  // Cards acting "during the token placement phase" (Martyr Meter, Dokstok, S.L.A.K.).
...
export function takeoverOptions(s: GameState, playerId: string): { card: string; onto: string; side: Side }[] {
  ...
  for (const card of candidates) {
    ...
    for (const spot of useSpots) if (!puppetForbidden(s, card, spot.onto, playerId)) out.push({ card, ...spot });
  }
  return out;
}
...
function placeTree(s: GameState, iid: string, controller: string, m: string, sd: Side, layout: TreeLayout, res: MoveResult,
  ...
  for (const { child, local } of layout[iid] ?? []) {
    ...
    const open = freeArrowsFor(s, controller, iid) ? catacombsSides(s, iid, new Set()) : openArrows(s, iid);

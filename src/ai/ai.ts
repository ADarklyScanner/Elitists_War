...
import {
  ...
  type AiLevel, GOAL_PROGRESS, moveSubtree, placeGroup, isPrivileged,
} from '../engine';
...
function declarePlots(s: GameState, pl: string, a: Extract<Action, { type: 'attack' }>): PlotPlay[] {
  ...
  for (let round = 0; round < (P.smartTakeover ? 2 : 1) && cur.chance < 0.85; round++) {
    ...
    for (const c of plotsInHand(s, pl)) {
      ...
      for (const play of declareOptions(probe, pl, c, a.attacker)) {
 ...
 */
function declareOptions(probe: GameState, pl: string, card: string, attacker: string): PlotPlay[] {

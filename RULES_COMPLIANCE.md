...
| R009 Cancellation | A cancelled Plot never happened (and a cancel can itself be cancelled); cancelled attacker action returns helpers' tokens and their Plots to hand; attacks can be called off before the attacker commits a Plot. Outside attacks, moves, main-phase abilities, Relief, Resource plays, links and the Illuminati's Group draw are announced and can be cancelled before they happen (costs stay paid); buying Plots cannot be cancelled |
